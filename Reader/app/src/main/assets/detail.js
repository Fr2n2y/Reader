    ...
    if(videoData && Object.prototype.toString.call(videoData) == '[object String]'){
        ...
        content = handle_img_position(content, video_p, videos.length,
            function(i) {
                ...
                if(videos[i].title) {
                    videoItemContent += '<p style="margin-top:26px;" class="video_parent"><div class="video" '+getVideoRule(videos[i])+' videoType=normal imgsrc="'+videos[i].thumbnail+'" normalSrc="'+videos[i].video.Normal.src+'" HDSrc="'+videos[i].video.HD.src+'" duration="'+videos[i].duration+'" guid="'+videos[i].guid+'">'+loadMessage+'</div><div style="margin-top:11px;"><small class="slide-small" style="line-height:23px;">'+videos[i].title+'</small></div>';
                }else{
                    videoItemContent += '<p style="margin-top:26px;" class="video_parent"><div  class="video" '+getVideoRule(videos[i])+' videoType=normal imgsrc="'+videos[i].thumbnail+'" normalSrc="'+videos[i].video.Normal.src+'" HDSrc="'+videos[i].video.HD.src+'" duration="'+videos[i].duration+'" guid="'+videos[i].guid+'">'+loadMessage+'</div>';
                }
...
//    console.log('window.innerHeight  '+window.innerHeight+', document.body.clientHeight  '+document.body.clientHeight+', VIEWPORT_HEIGHT  '+VIEWPORT_HEIGHT);
    ...
    var videoJumpComponent = container.getElementsByClassName('video_button');
